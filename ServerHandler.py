# Embedded file name: sdcard/com.hipipal.qpyplus/scripts/inunxl@bs/ServerHandler.py
import sys
import select
import socket
import random
...
class ProxyHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def get_recv_headers(self):
        self.send_connection_logger('[+] Request:\r\n[+] From Adress - %s:%s\r\n[+] %s\r\n' % (self.client_address[0], self.client_address[1], self.requestline))
        for header, value in self.headers.items():
            self.send_connection_logger('[+] %s: Locked\r\n' % str(header).title())
    ...
    def get_send_inject(self):
        if sets.IQUERY:
            ...
            if self.phost and self.pport:
                self.send_connection_logger('[+] Using Proxy: Locked\r\n')
            elif sets.ADMODE:
            ...
            self.send_connection_logger('[+] Host: Hidden PetapaGenit\r\n\r\n')
    ...
    def send_connection_error(self, msg, page = True):
        ...
        self.send_connection_logger('[+] Connection Error: Locked\r\n')
