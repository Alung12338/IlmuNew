...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('---sabar--- '.format(self.proxy_host, self.proxy_port))
            ...
            self.log('---berhasil---   ''[CC]'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
...
def main():
    print(colors('\n'.join([
    	 '[G1]''SUBSCRIBE MY YOUTUBE CHANNEL','[CC]'
    	 '[G1]''all things','[CC]'
    	 '=============================================','[CC]'
     'Nama      : Alung','[CC]'
     'Github    : https://github.com/Alung12338','[R1]'
     'Script    : New Telkomsel RGIlmpedia','[CC]'
     'Penting   : Susah konek? on off data atau mode pesawat','[R1]'
     '=============================================','[CC]'
    ])))    
